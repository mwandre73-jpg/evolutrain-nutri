...
import { Plus, Search, Play, Trash2, Edit2, X, Dumbbell, Loader2 } from "lucide-react";
        ...
        <div className="space-y-8 animate-slide-up">
            <header className="flex items-center justify-between">
                <div>
                    ...
                    <p className="mt-2 text-zinc-600 dark:text-zinc-400">
                        Gerencie os vídeos e demonstrações para seus atletas.
                    </p>
                </div>
                <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredExercises.length === 0 ? (
                        <div className="col-span-full py-12 text-center text-zinc-500 bg-white rounded-3xl border-2 border-dashed border-zinc-200 dark:bg-zinc-900 dark:border-zinc-800">
                            ...
                            <p className="text-sm">Comece adicionando exercícios com vídeos demonstrativos.</p>
                        </div>
                    ) : (
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 animate-slide-up">
                        ...
                        <form onSubmit={handleSave} className="space-y-4">
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-zinc-500 uppercase ml-1">Nome do Exercício</label>
                                <input
                                    value={formData.name}
                                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                    className="w-full rounded-2xl bg-zinc-50 border-none px-4 py-3.5 focus:ring-2 focus:ring-brand-primary dark:bg-zinc-800 outline-none"
                                    placeholder="Ex: Agachamento Livre"
                                    required
                                />
                            </div>
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-zinc-500 uppercase ml-1">Músculos/Foco</label>
                                <input
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-zinc-500 uppercase ml-1">URL do Vídeo (Cloudinary)</label>
                                <input
                            ...
                            <button
                                type="submit"
                                disabled={isSaving}
                                className="w-full rounded-2xl premium-gradient py-4 font-bold text-white shadow-lg transition-all hover:opacity-90 active:scale-95 mt-4 disabled:opacity-50"
                            >
                                {isSaving ? "Salvando..." : editingId ? "Atualizar" : "Salvar Exercício"}
                            </button>
                        </form>
                    </div>
                </div>
